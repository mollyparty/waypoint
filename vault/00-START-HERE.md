...
1. **Answer the six Phase 9 gate cards** on `dashboard/index.html`, then record DEC-014 onward. Card 1 (scope and timeline) is the consequential one and it supersedes DEC-010's dates. Cards 2 to 4 are the founder agreement: equity split, vesting, option pool. Cards 5 and 6 are formation timing and the round.
...
- **The riskiest number in the whole repo is the 1.8x AI leverage multiplier**, which underwrites the schedule, the scope cut and the round size, and has never been measured on this team. The month-6 checkpoint exists to replace it with evidence; below 1.2x the plan is rebuilt rather than adjusted. #risk
