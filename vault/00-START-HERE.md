...
- **Scope source of truth:** **`catalog/features.json`** and its interactive surface `catalog/index.html` ([[DEC-020 Features catalog is the source of truth for release phase]], 2026-08-06). 70 entries — 30 features, 15 compliance requirements, 19 non-functional requirements, 6 items ruled out by DEC-006 — each tied to persona, need and job, with release assignment adjustable and the projected launch month recomputed live. `mvp-scope.md`, `prd.md` and `rice-prioritization.md` keep their analysis but no longer decide phase; all three are annotated. It corrected full scope to **~32-33 person-months from 29-31**, because the obligations were absorbed into a single ~2 pm overhead line. That correction is now propagated into the blueprint, whose **section 05 is generated from the catalog** ([[DEC-021 Blueprint features section is generated from the catalog]], 2026-08-30) — never hand-edit between the `CATALOG:START` / `CATALOG:END` markers in `blueprint/index.html` or `blueprint/blueprint.md`; run `python catalog/build.py` instead. See [[Features Catalog and Scope Arithmetic]].
...
- [x] **Phase 8 blueprint drafted** 2026-08-06: `blueprint/blueprint.md` and `blueprint/index.html`; sections 12 and 13 rewritten by Phase 9. **Section 05 rebuilt 2026-08-30** as the condensed features catalog, generated by `catalog/build.py` (DEC-021), with eight contradicting "29 to 31 person-months" figures corrected to 32 to 33 and the full-scope projection recomputed from 21-28 to 22-30 months
- [x] **Memory audit and remediation** 2026-08-06: graph coverage 86 of 86 documents (eleven had no node at all), the Phase 6 launch-shape contradiction fixed, Phases 7 and 8 split into their own session note, catalogs synced. The graphify post-commit hook is now scoped to application code paths, because its no-LLM rebuild was silently dropping curated nodes; the extension filter alone still let `catalog/build.py` through and cost 11 nodes, so `catalog/`, `scripts/` and `tools/` are excluded too (**local-only fix; `.git/hooks/` is untracked. Check `git status` for `graphify-out/` churn after every commit**)
...
1. **Review the five remaining Phase 9 cards**, starting with equity, vesting and the option pool as one founder agreement discussion. Scope card 1 is accepted under DEC-014 and already applied to the catalog and generated blueprint. Prepare the sprint-zero acceptance/dependency map in parallel.
2. **Founder review of the blueprint.** Its capital and team sections are rebuilt on real numbers and section 05 now carries the full features catalog, so the largest of the four open items in `research/08-blueprint/_index.md` is closed. Remaining three: whether to publish to a separate public Vercel project, whether an EN/ES edition is wanted, and the Runna ARR conflict between two source documents.
...
- **A scope decision made in the catalog UI lives in one browser only.** Scenario state is `localStorage`, so it must be exported and written back into `features.json` to become real. Likewise `catalog/build.py` is not wired to a hook, so editing the JSON without running it now leaves both `FEATURES.md` **and the investor blueprint** stale — which is exactly the drift the catalog was built to end. #risk
- **The blueprint is still two hand-maintained documents.** Section 05 is generated in both `blueprint/index.html` and `blueprint/blueprint.md`, but the other sixteen sections are duplicated prose that can drift apart. The eight stale scope figures fixed on 2026-08-30 were exactly that failure mode. #risk
