...
> Version-Timestamp: 2026-08-30 18:50:00 UTC-4
...
- **Blueprint section 05 is now the features catalog, and it is generated rather than written** (DEC-021). `catalog/build.py` injects a condensed view into both `blueprint/index.html` and `blueprint/blueprint.md` between `CATALOG:START` / `CATALOG:END` markers: all 30 features grouped by their eight modules, four columns each (ID and name, one-line purpose, release badge, personas with primaries emphasised), a stats strip whose counts are computed at build time, and the never-build list rendered from the catalog's six ruled-out entries. Per-feature person-months and RICE ranks stay out as internal planning detail; the two obligation-heavy modules collapse to counts rather than tabling 34 requirements in an investor document. Section heading and nav label changed to "The product and its features" and "Product and features". The blueprint block carries no timestamp, so a rebuild that changes nothing produces no diff in the document an investor reads.
...
- **The graphify post-commit hook's code filter now excludes repository tooling.** Scoping it to code extensions was not enough: committing `catalog/build.py`, a generator rather than application code, tripped the rebuild and it dropped the curated graph from 227 nodes to 216 before being restored from the commit. The filter now also excludes `catalog/`, `scripts/` and `tools/`, verified to still fire on application source and still skip doc-only commits. As before, `.git/hooks/` is untracked, so this does not travel to a fresh clone.
...
- **The blueprint was the last document still quoting the superseded feature arithmetic.** Its section 05 named 15 MVP features but tabled only 7, dumped the other 8 into a prose run-on, quoted "22.75 person-months plus roughly 2 months of release overhead" — the exact rounded line the catalog had corrected — and called the scope "a solo founder's MVP" three weeks after the team was defined as three part-time founders. Rebuilt from the catalog.
- **Eight contradicting scope figures corrected across the blueprint**, four in each file: "29 to 31 person-months" now reads 32 to 33, matching the catalog. The derived full-scope projection moves from 21-to-28 months to **22 to 30**, recomputed the way the original was derived (the 1.8x-to-2.5x leverage band applied to the corrected total, which reproduces the old range from the old total). The DEC-010 row in the blueprint's decision log keeps its original figure with a note that both its dates and its total are superseded, because rewriting it would falsify the record of what was decided.
- **The stale pre-seed open item in `research/08-blueprint/_index.md` is closed.** It still asked for a decision on a "~$350k to $400k pre-seed" that Phase 9 had already replaced with a $50k SAFE at a $1.5M cap. The file now also records that section 05 is generated, so nobody hand-edits inside the markers and wonders why their change vanished.
