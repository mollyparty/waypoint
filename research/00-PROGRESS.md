...
**A note on the graph.** The graphify post-commit hook has been scoped to code file extensions **excluding `catalog/`, `scripts/` and `tools/`**, because its rebuild runs without an LLM pass and drops curated nodes. Documentation commits no longer trigger it, and neither does repository tooling: the extension filter alone let `catalog/build.py` through and that rebuild cut the graph from 227 nodes to 216 before it was restored from the commit. Update the graph deliberately instead. **This fix lives in `.git/hooks/post-commit`, which git does not track**, so a fresh clone on another machine will not have it. After every commit, check `git status` for `graphify-out/` changes; if the graph lost nodes, `git checkout -- graphify-out/` restores it and the hook needs the exclusion reapplied.
